...
def poll_packet_queue(token: str):
    ...
    while True:
        ...
        if queue_size > 0 and queue_size % 20 == 0:
            log.debug("Current queue size: {}".format(queue_size))
        ...
        try:
            ...
            log.exception("Failed to parse: {}".format(raw_packet.summary()))
            continue

        try:
            upload_packet(url, token, parsed_packet)
        except Exception as exc:
            log.exception("Failed to upload: {}".format(exc))
...
if __name__ == "__main__":
    ...
    thread.start()
    log.info("Started queue polling")
